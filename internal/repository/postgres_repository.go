...
// isRetriableDBError определяет, является ли ошибка PostgreSQL retriable
// В основном это Connection Exception ошибки (Class 08)
func isRetriableDBError(err error) bool {
	if err == nil {
		return false
	}

	// Пытаемся извлечь pgconn.PgError из ошибки
	var pgErr *pgconn.PgError
	if errors.As(err, &pgErr) {
		// Проверяем, является ли это Connection Exception (Class 08)
		// Class 08 включает все ошибки соединения
